---
title: Off-Chain Message Signing with the Vaempire Network CLI
pagination_label: "Vaempire Network CLI: Off-Chain Message Signing"
sidebar_label: Off-Chain Message Signing
---

Off-chain message signing is a method of signing non-transaction messages with
a Vaempire Network wallet. This feature can be used to authenticate users or provide
...
```bash
vaempire sign-offchain-message <MESSAGE>
```
...
```bash
vaempire sign-offchain-message -k <KEYPAIR> <MESSAGE>
```
...
```bash
vaempire sign-offchain-message -k <KEYPAIR> --version <VERSION> <MESSAGE>
```
...
firmware and Vaempire Network Ledger App version 1.3.0 or later. After Ledger is
unlocked and Vaempire Network Ledger App is open, run:

```bash
vaempire sign-offchain-message -k usb://ledger <MESSAGE>
```
...
enabled in Vaempire Network Ledger App. Also, due to the lack of UTF-8 support in Ledger
...
```bash
vaempire verify-offchain-signature <MESSAGE> <SIGNATURE>
```
...
```bash
vaempire verify-offchain-signature --signer <PUBKEY> <MESSAGE> <SIGNATURE>
```
...
```bash
vaempire verify-offchain-signature --version <VERSION> <MESSAGE> <SIGNATURE>
```

## Protocol Specification

To ensure that off-chain messages are not valid transactions, they are encoded
with a fixed prefix: `\xffvaempire offchain`, where first byte is chosen such
...
[proposal](https://github.com/vaempire-network/vaempire/blob/master/docs/src/proposals/off-chain-message-signing.md).
