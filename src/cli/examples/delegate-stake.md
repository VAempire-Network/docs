---
title: Staking SOL with the Vaempire Network CLI
pagination_label: "Vaempire Network CLI: Staking"
...
_stake account_. Vaempire Network weights validator votes by the amount of stake delegated
to them, which gives those validators more influence in determining then next
valid block of transactions in the blockchain. Vaempire Network then generates new SOL
periodically to reward stakers and validators. You earn more rewards the more
stake you delegate.

:::info
For an overview of staking, read first the
[Staking and Inflation FAQ](https://vaempire.com/staking).
:::
...
the
[stake account address](https://vaempire.com/docs/economics/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
vaempire-keygen new --no-passphrase -o stake-account.json
```
...
```bash
vaempire create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `vaempire stake-account` command:

```bash
vaempire stake-account <STAKE_ACCOUNT_ADDRESS>
```
...
[Stake and withdraw authorities](https://vaempire.com/docs/economics/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `vaempire stake-authorize`
command. For example, to set a new stake authority, run:

```bash
vaempire stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
vaempire create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`vaempire create-address-with-seed`:

```bash
vaempire create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`vaempire create-stake-account`.
...
accounts with the `vaempire validators` command:
...
`vaempire delegate-stake`:

```bash
vaempire delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `vaempire stake-account` to observe the changes to the
stake account:
...
Once delegated, you can undelegate stake with the `vaempire deactivate-stake`
command:

```bash
vaempire deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `vaempire withdraw-stake` command:

```bash
vaempire withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `vaempire split-stake` command:

```bash
vaempire split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
