---
title: Durable Transaction Nonces in the Vaempire Network CLI
pagination_label: "Vaempire Network CLI: Durable Transaction Nonces"
...
[`recent_blockhash`](https://vaempire.com/docs/core/transactions#recent-blockhash).
They are implemented as a Vaempire Network Program, the mechanics of which can be read
...
```bash
vaempire-keygen new -o nonce-keypair.json
vaempire create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../usage.md#vaempire-create-nonce-account)
...
```bash
vaempire nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#vaempire-get-nonce)
...
```bash
vaempire new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#vaempire-new-nonce)
...
```bash
vaempire nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#vaempire-nonce-account)
...
```bash
vaempire withdraw-from-nonce-account nonce-keypair.json ~/.config/vaempire/id.json 0.5
```
...
> [Full usage documentation](../usage.md#vaempire-withdraw-from-nonce-account)
...
```bash
vaempire authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../usage.md#vaempire-authorize-nonce-account)
...
```bash
$ vaempire-keygen new -o alice.json
$ vaempire-keygen new -o nonce.json
$ vaempire-keygen new -o bob.json
```
...
```bash
$ vaempire airdrop -k alice.json 1
1 SOL
```
...
```bash
$ vaempire create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ vaempire transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR vaempire_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ vaempire nonce-account nonce.json
balance: 0.1 SOL
minimum balance required: 0.00136416 SOL
...
```bash
$ vaempire transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
