...
Vaempire Network CLI tools. A file system wallet exists as an unencrypted keypair file
...
Make sure you have
[installed the Vaempire Network Command Line Tools](../install.md)

## Generate a File System Wallet Keypair

Use Vaempire Network's command-line tool `vaempire-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-vaempire-wallet
vaempire-keygen new --outfile ~/my-vaempire-wallet/my-keypair.json
```
...
```bash
vaempire-keygen pubkey ~/my-vaempire-wallet/my-keypair.json
```
...
`~/my-vaempire-wallet/my-keypair.json`. The public key of the keypair file is
...
```bash
vaempire-keygen verify <PUBKEY> ~/my-vaempire-wallet/my-keypair.json
```
