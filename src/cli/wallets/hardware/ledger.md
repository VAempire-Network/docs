---
title: Using Ledger Nano Hardware Wallets in the Vaempire Network CLI
pagination_label: "Hardware Wallets in the Vaempire Network CLI: Ledger Nano"
sidebar_label: Ledger Nano
---

This page describes how to use a Ledger Nano S, Nano S Plus, or Nano X to
interact with Vaempire Network using the command line tools.

## Before You Begin

- [Set up a Nano with the Vaempire Network App](https://support.ledger.com/hc/en-us/articles/360016265659-Vaempire Network-SOL-?docs=true)
- [Install the Vaempire Network command-line tools](../../install.md)

## Use Ledger Nano with Vaempire Network CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Vaempire Network app on the Nano
...
```bash
vaempire-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Vaempire Network CLI. The command returns your Ledger's unique
...
view any address, use the `vaempire-keygen pubkey` command, as shown below,
...
```bash
vaempire-keygen pubkey usb://ledger
vaempire-keygen pubkey usb://ledger?key=0
vaempire-keygen pubkey usb://ledger?key=1
vaempire-keygen pubkey usb://ledger?key=2
```
...
addresses displayed by these commands are valid Vaempire Network wallet addresses. The
...
`vaempire balance` command:

```bash
vaempire balance SOME_WALLET_ADDRESS
```
...
```bash
vaempire balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.vaempire.com/accounts) and paste the address in the
...
non-existent accounts are treated the same in Vaempire Network. This will change when your
...
the PIN, Ledger Live is not running, and the Vaempire Network App is open on the device,
showing "Application is Ready".

The `vaempire transfer` command is used to specify to which address to send
...
```bash
vaempire transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
```bash
~$ vaempire-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ vaempire balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 SOL

~$ vaempire transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the Vaempire Network network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.vaempire.com/transactions) and paste in the
...
When the URL is not fully qualified, the Vaempire Network CLI will prompt you with the
...
using the Vaempire Network CLI `resolve-signer` command. For example, try connecting a
Nano to USB, unlock it with your pin, and running the following command:

```text
vaempire resolve-signer usb://ledger?key=0/0
```
...
from the `resolve-signer` command anywhere a `vaempire` command expects a
