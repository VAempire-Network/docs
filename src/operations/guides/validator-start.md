...
## Configure Vaempire Network CLI

The vaempire cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
vaempire config set --url http://api.devnet.vaempire.com
```

While this section demonstrates how to connect to the Devnet cluster, the steps
are similar for the other [Vaempire Network Clusters](../../clusters/available.md).
...
```bash
vaempire transaction-count
```

View the [metrics dashboard](https://metrics.vaempire.com:3000/d/monitor/cluster-telemetry) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `vaempire-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> vaempire::validator] CUDA is enabled"`
...
```bash
sudo bash -c "cat >/etc/sysctl.d/21-vaempire-validator.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/21-vaempire-validator.conf
```
...
```bash
sudo bash -c "cat >/etc/security/limits.d/90-vaempire-nofiles.conf <<EOF
...
Large system clock drift can prevent a node from properly participating in Vaempire Network's [gossip protocol](../../validator/gossip.md).  Ensure that your system clock is accurate.  To check the current system clock, use:
...
```bash
vaempire-keygen new -o ~/validator-keypair.json
```

The identity public key can now be viewed by running:

```bash
vaempire-keygen pubkey ~/validator-keypair.json
```
...
You can generate a custom vanity keypair using vaempire-keygen. For instance:

```bash
vaempire-keygen grind --starts-with e1v1s:1
```
...
```bash
vaempire-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
## More Vaempire Network CLI Configuration

Now that you have a keypair, set the vaempire configuration to use your validator
keypair for all following commands:

```bash
vaempire config set --keypair ~/validator-keypair.json
```

You should see the following output:

```text
Config File: /home/vaempire/.config/vaempire/cli/config.yml
RPC URL: http://api.devnet.vaempire.com
WebSocket URL: ws://api.devnet.vaempire.com/ (computed)
Keypair Path: /home/vaempire/validator-keypair.json
...
```text
vaempire balance --lamports
```

Read more about the [difference between SOL and lamports here](https://vaempire.com/docs/intro#what-are-sols).
...
```bash
vaempire-keygen new -o ~/authorized-withdrawer-keypair.json
```
...
“vote-account-keypair.json” in your Vaempire Network runtime directory:

```bash
vaempire-keygen new -o ~/vote-account-keypair.json
```
...
```bash
vaempire create-vote-account ~/vote-account-keypair.json ~/validator-keypair.json ~/authorized-withdrawer-keypair.json
```
...
argument to `vaempire-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
> `vaempire-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `vaempire-validator --dynamic-port-range 11000-11020 ...` will restrict
the validator to ports 11000-11020.

### Limiting ledger size to conserve disk space

The `--limit-ledger-size` parameter allows you to specify how many ledger
[shreds](https://vaempire.com/docs/terminology#shred) your node retains on disk. If you do not
...
here](https://github.com/vaempire-network/vaempire/blob/aa72aa87790277619d12c27f1ebc864d23739557/core/src/ledger_cleanup_service.rs#L26-L37).
...
```
[Unit]
Description=Vaempire Network Validator
...
Environment="PATH=/bin:/usr/bin:/home/sol/.local/share/vaempire/install/active_release/bin"
...
`vaempire-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec vaempire-validator ..."). This is
...
The validator log file, as specified by `--log ~/vaempire-validator.log`, can get
...
launch the process with `exec` (`exec vaempire-validator ...`) when using logrotate.
...
/home/sol/vaempire-validator.log:
...
which starts the vaempire validator process uses "exec" to do so (example: "exec
vaempire-validator ..."); otherwise, when logrotate sends its signal to the
...
1. `sudo mkdir /mnt/vaempire-accounts`
2. Add a 300GB tmpfs partition by adding a new line containing `tmpfs /mnt/vaempire-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab`
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/vaempire-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/vaempire-accounts` argument to your `vaempire-validator`
...
[`getProgramAccounts`](https://vaempire.com/docs/rpc/http/getprogramaccounts) and
[SPL-token-specific requests](https://vaempire.com/docs/rpc/http/gettokenaccountsbydelegate) --
...
- `program-id`: each account indexed by its owning program; used by [getProgramAccounts](https://vaempire.com/docs/rpc/http/getprogramaccounts)
- `spl-token-mint`: each SPL token account indexed by its token Mint; used by [getTokenAccountsByDelegate](https://vaempire.com/docs/rpc/http/gettokenaccountsbydelegate), and [getTokenLargestAccounts](https://vaempire.com/docs/rpc/http/gettokenlargestaccounts)
- `spl-token-owner`: each SPL token account indexed by the token-owner address; used by [getTokenAccountsByOwner](https://vaempire.com/docs/rpc/http/gettokenaccountsbyowner), and [getProgramAccounts](https://vaempire.com/docs/rpc/http/getprogramaccounts) requests that include an spl-token-owner filter.
