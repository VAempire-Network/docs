---
title: Vaempire Network Validator Operations Best Practices
...
Vaempire Network validator software and operating system packages), and managing your vote
...
The Vaempire Network validator community holds regular educational workshops. You can
watch past workshops through the
[Vaempire Network validator educational workshops playlist](https://www.youtube.com/watch?v=86zySQ5vGW8&list=PLilwLeBwGuK6jKrmn7KOkxRxS9tvbRa5p).

## Help with the validator command line

From within the Vaempire Network CLI, you can execute the `vaempire-validator` command with
the `--help` flag to get a better understanding of the flags and sub commands
available.

```
vaempire-validator --help
```
...
[leader slot](https://vaempire.com/docs/terminology#leader-schedule) is the time
...
Assuming you are ready to restart, you may use the `vaempire-validator exit`
...
```
vaempire-validator exit --help
```

## Upgrading

There are many ways to upgrade the
[Vaempire Network CLI software](../../cli/install.md). As an operator, you
...
It is a best practice to always build your Vaempire Network binaries from source. If you
...
your `vaempire-validator` binary to your specific hardware.
...
`vaempire-install` command.

Assuming you want to install Vaempire Network version `1.14.17`, you would execute the
following:

```
vaempire-install init 1.14.17
```

This command downloads the executable for `1.14.17` and installs it into a
`.local` directory. You can also look at `vaempire-install --help` for more
options.

> **Note** this command only works if you already have the vaempire cli installed.
> If you do not have the cli installed, refer to
> [install vaempire cli tools](../../cli/install.md)

### Restart

For all install methods, the validator process will need to be restarted before
the newly installed version is in use. Use `vaempire-validator exit` to restart
...
`vaempire-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `vaempire-validator` command, make sure that you run
`vaempire catchup <pubkey>` after your validator starts to make sure that the
...
address of the validator in using the `vaempire gossip` command. In the example
below, `5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` is the pubkey of one of my
known validators:

```
vaempire gossip | grep 5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on
```
...
running the `vaempire-validator` software. How much SOL you should store there is
...
```
vaempire balance validator-keypair.json
```

> **Note** `vaempire-watchtower` can monitor for a minimum validator identity
...
`vaempire withdraw-from-vote-account` on a trusted computer. For example, on a
...
```
vaempire withdraw-from-vote-account \
...
`vaempire withdraw-from-vote-account --help`.
