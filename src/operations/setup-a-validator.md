---
title: Setup a Vaempire Network Validator
sidebar_label: Setup a Validator
sidebar_position: 5
---

This is a guide for getting your validator setup on the Vaempire Network testnet cluster
for the first time. Testnet is a Vaempire Network cluster that is used for performance
...
## Install The Vaempire Network CLI Locally

To create your validator vote account, you need to install the
[Vaempire Network command line interface](../cli/index.md) on your local computer.

You can either use
[Vaempire Network's Install Tool](../cli/install.md#use-vaempires-install-tool) section from
...
Once the Vaempire Network CLI is installed, you can return to this document once you are
...
```
vaempire-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
You should see a line that says: `RPC URL: https://api.testnet.vaempire.com`
...
Before you can create your vote account, you need to configure the Vaempire Network
command line tool a bit more.

The below command sets the default keypair that the Vaempire Network CLI uses to the
...
Now, use the Vaempire Network cluster to create a vote account.
...
```
vaempire create-vote-account -ut \
...
> transaction fees. Both flags are not necessary if you configured the vaempire
...
## Install The Vaempire Network CLI on Remote Machine

Your remote machine will need the Vaempire Network cli installed to run the validator
software. Refer again to
[Vaempire Network's Install Tool](../cli/install.md#use-vaempires-install-tool) or
...
exec vaempire-validator \
    ...
    --only-known-rpc \
    --log /home/sol/vaempire-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
...
Refer to `vaempire-validator --help` for more information on what each flag is
...
The script should execute the `vaempire-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep vaempire-validator
```

You should see a line in the output that includes `vaempire-validator` with all
...
Gossip is a protocol used in the Vaempire Network clusters to communicate between
...
The command `vaempire gossip` lists all validators that have registered with the
protocol. To check that the newly setup validator is in gossip, we will `grep`
for our pubkey in the output:

```
vaempire gossip | grep <pubkey>
```
...
### Vaempire Network Catchup

The `vaempire catchup` command is a useful tool for seeing how quickly your
validator is processing blocks. The Vaempire Network network has the capability to produce
...
The `vaempire catchup` command is a tool that tells you how far behind the network
your validator is and how quickly you are catching up:

```
vaempire catchup <pubkey>
```

If you see a message about trying to connect, your validator may not be part of
the network yet. Make sure to check the logs and double check `vaempire gossip`
and `vaempire validators` to make sure your validator is running properly.
...
the commands mentioned earlier to check gossip and Vaempire Network validators:

```
tail -f /home/sol/vaempire-validator*.log
```

## Monitoring

`vaempire-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#vaempire-watchtower)
using Vaempire Network Watchtower here in the docs.
