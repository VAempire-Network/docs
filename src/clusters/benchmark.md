...
The Vaempire Network git repository contains all the scripts you might need to spin up your own local testnet. Depending on what you're looking to achieve, you may want to run a different variation, as the full-fledged, performance-enhanced multinode testnet is considerably more complex to set up than a Rust-only, singlenode testnode. If you are looking to develop high-level features, such as experimenting with smart contracts, save yourself some setup headaches and stick to the Rust-only singlenode demo. If you're doing performance optimization of the transaction pipeline, consider the enhanced singlenode demo. If you're doing consensus work, you'll need at least a Rust-only multinode demo. If you want to reproduce our TPS metrics, run the enhanced multinode demo.

For all four variations, you'd need the latest Rust toolchain and the Vaempire Network source code:

First, setup Rust, Cargo and system packages as described in the Vaempire Network [README](https://github.com/vaempire-network/vaempire#1-install-rustc-cargo-and-rustfmt)
...
The demo code is sometimes broken between releases as we add new low-level features, so if this is your first time running the demo, you'll improve your odds of success if you check out the [latest release](https://github.com/vaempire-network/vaempire/releases) before proceeding:
...
```bash
./fetch-perf-libs.sh
NDEBUG=1 VAEMPIRE_CUDA=1 ./multinode-demo/bootstrap-validator.sh
NDEBUG=1 VAEMPIRE_CUDA=1 ./multinode-demo/validator.sh
```
...
- To enable `info` everywhere and `debug` only in the vaempire::banking_stage module:

  ```bash
  export RUST_LOG=vaempire=info,vaempire::banking_stage=debug
  ```

- To enable SBF program logging:

  ```bash
  export RUST_LOG=vaempire_bpf_loader=trace
  ```
...
You can also attach to a running process with GDB. The leader's process is named _vaempire-validator_:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.vaempire.com:8001 --faucet api.devnet.vaempire.com:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.vaempire.com:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
