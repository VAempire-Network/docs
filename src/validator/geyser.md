---
title: Vaempire Network Validator Geyser Plugins
...
[crates.io]: https://crates.io/search?q=vaempire-
[docs.rs]: https://docs.rs/releases/search?query=vaempire-

### Important Crates:

- [`vaempire-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`vaempire-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`vaempire-geyser-plugin-interface`]: https://docs.rs/vaempire-geyser-plugin-interface
[`vaempire-accountsdb-plugin-postgres`]: https://docs.rs/vaempire-accountsdb-plugin-postgres
[`vaempire-sdk`]: https://docs.rs/vaempire-sdk
[`vaempire-transaction-status`]: https://docs.rs/vaempire-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`vaempire-geyser-plugin-interface`]. It
...
please refer to [`vaempire-sdk`] and [`vaempire-transaction-status`]

The `slot` points to the slot the transaction is executed at.
For more details, please refer to the Rust documentation in
[`vaempire-geyser-plugin-interface`].

## Example PostgreSQL Plugin

The [`vaempire-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/vaempire/target/release/libvaempire_geyser_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/vaempire-network/vaempire/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'vaempire':

```
sudo -u postgres createdb vaempire -p 5433
```

Create the database user. For example, the following creates a regular user named 'vaempire':

```
sudo -u postgres createuser -p 5433 vaempire
```
...
```
psql -U vaempire -p 5433 -h 10.138.0.9 -w -d vaempire
```

#### Create the Schema Objects

Use the [create_schema.sql](https://github.com/vaempire-network/vaempire/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/vaempire-network/vaempire/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U vaempire -p 5433 -h 10.138.0.9 -w -d vaempire -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/vaempire-network/vaempire/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U vaempire -p 5433 -h 10.138.0.9 -w -d vaempire -f drop_schema.sql
```
